...
class ShippingSchedule extends Component {
  ...
  toReceivedSchedule = async () => {
    this.setState({
      isLoadingConfirmDone: true,
    })
    try {
      await ServiceManagement.updateStatus(this.state.scheduleId, 6);
        ...
        <Row>
          <Col xs="12" lg="12">
            ...
            <Modal isOpen={openConfirmDoneSchedule} toggle={this.openConfirmDoneSchedule}>
              ...
              <ModalFooter>
                {
                  isLoadingConfirmDone ? <Button disabled color="primary">...Loading</Button> : <Button color="primary" onClick={this.toReceivedSchedule}>Xác nhận</Button>
                }
